...
include '../koneksi.php';
		...
		$html = file_get_html('https://eventkampus.com/event/kategori/seminar');
	...
	foreach($html->find('div[class=col-12 col-sm-6 col-md-4 col-lg-3 mb-2]') as $element)
		...
		if($d > 0){
			mysqli_query($koneksi, "
				update event set tanggal='$tanggal[$no]',lokasi='$lokasi[$no]', jenis='Seminar' where nama_event='$nama_event[$no]'");
			
		
		} else {
			mysqli_query($koneksi,"insert into event (nama_event, jenis, tanggal, lokasi, link_detail, gambar) values ('$nama_event[$no]','Seminar','$tanggal[$no]',

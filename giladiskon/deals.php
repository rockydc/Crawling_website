...
include '../koneksi.php';
		...
		$html = file_get_html('https://giladiskon.com/deals');
		...
		if($d > 0){
			...
			mysqli_query($koneksi, "
				update event set nama_event='$nama_event[$no]', tanggal='$tanggal[$no]', jenis='$jenis',gambar='$image[$gbr]' where nama_event='$nama_event[$no]'");
			echo 'update '.$image[$gbr];
			$no++;
			$gbr++;
		}else{
			mysqli_query($koneksi,"insert into event (id_event, nama_event, jenis, tanggal, lokasi, link_detail, gambar)values('','$nama_event[$no]','$jenis','$tanggal[$no]','','$link[$no]','$image[$gbr]')");
			$no++;
			$gbr++;
			echo 'save '.$image[$gbr];
		...
		// header("location:alat-olahraga.php");

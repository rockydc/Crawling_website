...
include 'koneksi.php';
		...
		$html = file_get_html('http://eventbanget.com/jobfair/');
		...
		if($d > 0){
			...
			mysqli_query($koneksi,"insert into event (id_event, nama_event, jenis, tanggal, link_detail, gambar)
				values('','$nama_event[$no]','$jenis[$no]','$mulai[$no]','$link[$no]','$image[$no]')");
			$no++;
		}
